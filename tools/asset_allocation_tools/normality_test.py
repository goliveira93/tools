"""
Faz o teste de normalidade dos benchmarks de mercado
Gera planilha de covariancia para cenários
"""
...
def add_qq_fig(dados, fig, title, row):
    ...
    if st.pvalue<0.05:
        fig.add_scatter(x=qq[0][0], y=qq[0][1], marker_color=colors[7], mode='markers+text',col=2,row=row, showlegend=False)
        fig.layout.annotations[row*2-1].update(text="NOT SHAPIRO NORMAL")
    else:
        fig.add_scatter(x=qq[0][0], y=qq[0][1], marker_color=colors[5], mode='markers',col=2,row=row, showlegend=False)
...
def get_historical_returns(startDate:datetime, endDate:datetime, offshore:bool=True)->pd.DataFrame:
    if offshore is True:
        ...
        basket.append({"Name":"Deflator","Ticker":"BZPIIPCA Index", "Source":"Bloomberg"})
        deflator="BZPIIPCA Index"
...
if __name__ == "__main__":
    if len(sys.argv)<1:
        ...
        while n<=rows:
            ...
            fig.add_trace(go.Histogram(x=rets[rets.columns[n-1]], name=rets.columns[n-1], showlegend=False,marker_color=colors[n]),row=n,col=1)
